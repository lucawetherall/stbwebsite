...
Note that the `pages` fields in `config.yml` are shared through the YAML anchor `&pf`, but the
**Worship** entry spells its fields out separately — a new page field has to be added in *both*
places or it will be missing on that one page.
...
- **SEO surfaces — change deliberately, they are load-bearing:** `src/lib/seo.ts` builds the one
  JSON-LD `@graph` every page emits (parish, website, standing services, breadcrumbs) — page-level
  nodes are passed to `Base` via the `jsonLd` prop and must *not* re-declare `@context` or repeat
  the parish address; reference `CHURCH_ID` instead. `src/lib/breadcrumbs.ts` derives the trail
  from `src/data/nav.ts`, so moving an item in the menu moves its breadcrumb too (tested).
  `scripts/sitemap-meta.mjs` reads `lastmod` out of content frontmatter, never file mtime — a CI
  checkout would otherwise date the whole archive to this morning.
